...
pub fn connect(pool: DBPool, tv: &gtk::TreeView)  {
    ...
    use diesel::{QueryDsl, GroupByDsl, RunQueryDsl, ExpressionMethods};
    
    let model = gtk::TreeStore::new(&[String::static_type()]);
    let db = pool.get().unwrap();
    let res: Vec<String> = tracks.select(artist).order(artist).group_by(artist).load(db.deref()).expect("Error in db connection");
